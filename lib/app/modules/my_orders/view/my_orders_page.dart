...
class _MyOrdersPageState extends State<MyOrdersPage> with LoaderMixin, MessagesMixin {
  final formatNumberPrice = NumberFormat.currency(name: 'R\$', locale: 'pt_BR', decimalDigits: 2);
        ...
        onRefresh: () => context.read<MyOrdersController>().findAll(),
          ...
          builder: (_, controller, __) {
            ...
            if (!isNull(controller.error)) {
              return ListView(
                children: [
                  Center(child: Text('Erro ao buscar pedidos tente novamente mais tarde')),
                  RaisedButton(
  ...
  String _calculateTotalOrder(OrderModel order) {
    final totalOrder = order.items.fold(0.0, (totalValue, orderItem) => totalValue += orderItem.item.price);
